...
async function Ready()
{
    ...
    function IframeBuffer_AND_AwaitngToInitialize_SYNERGY_RTM(iframe_buffer_stack, awaiting_for_mouseenter_to_initialize, iframe_buffer_slider, try_to_load_on_intersection_beta)
    {
        ...
        awaiting_for_mouseenter_to_initialize.addEventListener('change', function (e)
        {
            ...
            toggleTryingBtn_VisualFeedback(checked);
        ...
        try_to_load_on_intersection_beta.addEventListener('change', (e) =>
        {
            ...
            {
                AwaitingBtn_VisualFeedback(true, true);
            }
            else
            {
            ...
            iframe_buffer_stack.dispatchEvent(new Event('change'))
        });
...
function ObserveIframesAndDelployYTPlayers(targetClass)
{
    ...
    {
        onYouTubePlayerAPIReady(component, targetClass, 'initialized', 'first wave');
    }
    ...
    {
        intersect_obj.targetEl = component;
        intersect_obj.message = 'IntersectionObserver';
        // I'm quite impressed with this... I mean...
        window.YT_GIF_OBSERVERS.masterIntersectionObservers.push(ObserveIntersectToSetUpPlayer(intersect_obj));
    }
    ...
    function mutation_callback(mutationsList, observer)
    {
        ...
        for (const node of found)
        {
            ...
            {
                intersect_obj.targetEl = node;
                intersect_obj.message = 'MutationObserver';
                window.YT_GIF_OBSERVERS.masterIntersectionObservers.push(ObserveIntersectToSetUpPlayer(intersect_obj));
            }
        }
    };
    function Intersection_cb(catch_obj)
    {
        const { targetEl, targetClass, message, dataCreation } = catch_obj;
        onYouTubePlayerAPIReady(targetEl, targetClass, dataCreation, message);
    }
...
function ObserveIntersectToSetUpPlayer({ targetEl, targetClass, Intersection_cb, threshold, message, isIntersecting: hardcodeIntersection })
{
    // ObserveIntersectToSetUpPlaye when cssClass is added to the DOM
    const yobs = new IntersectionObserver(Intersection_callback, { threshold: threshold });
    ...
    function Intersection_callback(entries)
    {
        const pass_obj = {
            ...
            targetClass: targetClass,
            message: message,
            dataCreation: targetEl.getAttribute(attrInfo.creation),
            entry: null,
            entriesArr: [],
            yobs
        }
        ...
        {
            pass_obj.entry = entry;
            pass_obj.entriesArr.push(entry);
            if (entry.isIntersecting && hardcodeIntersection === undefined)
            {
                Intersection_cb(pass_obj);
                yobs.disconnect();
                break;
            }
        }

        if (hardcodeIntersection == 'anyways')
        {
            Intersection_cb(pass_obj);
        }
    }

    yobs.observe(targetEl);
...
async function onYouTubePlayerAPIReady(wrapper, targetClass, dataCreation, message = 'I dunno')
{
    ...
    // 5. 
    if (isValidAwaitingBtnCheck() || dataCreation == 'displaced-force-awaiting')
    {
...
function isValidAwaitingBtnCheck()
{
    const { awaiting_for_mouseenter_to_initialize } = UI.experience;
    return awaiting_for_mouseenter_to_initialize.checked && !awaiting_for_mouseenter_to_initialize.disabled;
}
...
async function onPlayerReady(event)
{
    ...
    PushNew_ShiftAllOlder_IframeBuffer(parentCssPath, parent); // push[len-1] -> shift[0] 
...
function PushNew_ShiftAllOlder_IframeBuffer(parentCssPath, parent)
{
    // 0.
    let arr = window.YT_GIF_OBSERVERS.masterIframeBuffer;
    const cap = getBufferCap();
    ...
    {
        // 2. while...
        const { shiftedArr, atLeastOne, lastOne } = FitIframeBuffer(arr, cap, parentCssPath, parent, 'buffer');
        arr = shiftedArr;
        // 3. mix and match
    ...
    {
        creationTypesYTGIF(arr, cap, parentCssPath, parent);
...
function FitIframeBuffer(arr, cap, parentCssPath, parent, creation)
{
    ...
    while (arr.length > cap)
    {
        ...
        if (parentCssPath != lastOne)
        {
            if (wrapper)
            {
                let hardcodedCreation = creation;
                if (UTILS.isElementVisible(wrapper))
                {
                    hardcodedCreation = 'displaced-force-awaiting';
                }
                CleanAndBrandNewWrapper(wrapper, attrInfo.creation, hardcodedCreation);
            }
            arr.shift(lastOne);
            atLeastOne = true;
        }
...
function getBufferCap()
{
    return parseInt(UI.range.iframe_buffer_slider.value, 10);
}

function toggleTryingBtn_VisualFeedback(checked)
{
    if (checked)
    {
        TryingBtn_VisualFeedback(true, true);
    }

    else
    {
        TryingBtn_VisualFeedback(false, false);
    }
}
...
function AwaitingBtn_Dispatch_ActiveCheck(bol)
{
    ...
    toggleTryingBtn_VisualFeedback(!bol);
...
{
    // const withFocus = UTILS.inViewportElsHard(document.querySelectorAll('.yt-gif-wrapper.dont-focus-block'));
    // const fromBuffer = UTILS.inViewportElsHard(document.querySelectorAll('[data-creation="buffer"]'));
    //const yt_target = UTILS.inViewportElsHard(document.querySelectorAll('.rm-xparser-default-yt-gif'));
    const anyLoaded = document.querySelectorAll('.yt-gif-wrapper');

    if (anyLoaded.length > cap)
    {
        let newCreation = 'displaced';
        let atLeastOneFunc = false;

        for (let i = 0; i < anyLoaded.length; i++)
        {
            const el = anyLoaded[i];
            const creation = anyLoaded[i].getAttribute(attrInfo.creation);


            if (i < (anyLoaded.length - cap))
            {
                const observer = new window.IntersectionObserver(([entry]) =>
                {
                    ...
                    if (el.parentNode)
                    {
                        const { shiftedArr, atLeastOne, lastOne } = FitIframeBuffer(arr, cap, parentCssPath, parent, newCreation);
                        window.YT_GIF_OBSERVERS.masterIframeBuffer = shiftedArr;
                    }
                    //slicedTarget = 
                    //CleanAndBrandNewWrapper(el, attrInfo.creation, newCreation);
                    observer.disconnect();
                    //atLeastOneFunc = atLeastOne;
                    return;
                    // it's almost working, so far it always removes anything that's not visible....
                    // so create a while loop to remove entries in window.YT_GIF_OBSERVERS.masterIframeBuffer
        ...
        // // reverse for loop
        // for (let i = anyLoaded.length - 1; i >= 0; i--)
        // {
        //     const element = anyLoaded[i];
        //     if (i > 0)
        //         CleanAndBrandNewWrapper(element, attrInfo.creation, newCreation);
        //     return;
        // }
    }
